...
class HttpRecorderTest(unittest.TestCase):
    ...
    def test_json_payload(self, mock_session, mock_span_to_json):
        test_timeout = 1.0
        recorder = HaystackHttpRecorder(self.a_url, timeout_seconds=test_timeout, use_json_payload=True)
    ...
    @mock.patch("haystack.http_recorder.FuturesSession")
    def test_timeout_and_payload_defaults(self, mock_futures_session, mock_span_to_proto):
        recorder = HaystackHttpRecorder(self.a_url,)
